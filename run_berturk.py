"""
Aşama 3: BERTurk Fine-Tuning
RTX 3060 Laptop 6GB VRAM için optimize edilmiş
"""
...
log("DataLoader tamam.")

log(f"Model yukleniyor: {MODEL_NAME}")
model = BertForSequenceClassification.from_pretrained(MODEL_NAME, num_labels=NUM_LABELS)
log("Model CPU'da tamam.")
model.to(DEVICE)
log(f"Model GPU'ya tasindi. VRAM: {torch.cuda.memory_allocated()/1024**3:.2f} GB")

total_params = sum(p.numel() for p in model.parameters()) / 1e6
log(f"Parametre: {total_params:.1f}M")
...
def evaluate(loader):
    ...
    return acc, f1, all_preds, all_labels, all_probas
...
with open(RESULTS_DIR / "berturk_results.json", "w") as f:
