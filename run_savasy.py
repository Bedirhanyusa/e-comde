"""
savasy/bert-turkish-text-classification — v4 verisi uzerinde fine-tuning
Bu model zaten Turkce sentiment icin on-egitilmis; bizim clean v4 datamizla
ustune egiterek guclu bir 3. ensemble modeli elde ediyoruz.
Hedef: >%82 macro-F1 (tek basina)
"""
...
MAX_LEN     = 192
SEED        = 42
LR          = 3e-6   # zaten fine-tuned oldugu icin cok dusuk LR
EPOCHS      = 8
BATCH_SIZE  = 8
...
CKPT_DIR    = Path("models/savasy_finetuned")
...
# ── Model — classifier head'i yeniden baslat ─────────────────────
...
log(f"\nEgitim basliyor | LR={LR} | Eff.batch={BATCH_SIZE*ACCUM} | AMP={USE_AMP}")
...
with open(RESULTS_DIR / "savasy_results.json", "w", encoding="utf-8") as f:
    json.dump(results, f, indent=2, ensure_ascii=False)
log("Sonuclar: results/savasy_results.json")
log("\nEGITIM TAMAMLANDI (savasy).")
