"""
BERTurk v3b — Mevcut checkpoint'ten devam, v4 temiz verisi ile ince ayar
- models/berturk_finetuned'dan basla (F1=0.7538, zaten domain-adapted)
- v4 temiz dengeli veri (49K, Trendyol dahil) ile yeniden egit
- MAX_LEN=192, LR=8e-6, cosine schedule, patience=5
"""
...
BASE_CKPT   = Path("models/berturk_finetuned")
...
log(f"Cihaz: {DEVICE} | AMP: {USE_AMP}")
...
if not v4_dir.exists():
    log("HATA: data/processed_v4 bulunamadi. Once prepare_data_v4.py calistirin.")
    exit(1)
...
# ── Tokenizer: once checkpoint dene, yoksa HuggingFace'den indir ─
if BASE_CKPT.exists():
    tokenizer = AutoTokenizer.from_pretrained(str(BASE_CKPT))
    log(f"Tokenizer checkpoint'ten yuklendi: {BASE_CKPT}")
else:
    tokenizer = AutoTokenizer.from_pretrained(MODEL_NAME)
    log(f"Tokenizer HuggingFace'den yuklendi: {MODEL_NAME}")
...
class ReviewDataset(Dataset):
    def __init__(self, df):
        ...
        for i in range(0, len(texts), 256):
            ...
            if "token_type_ids" in enc: tti_list.append(enc["token_type_ids"])
    ...
    def __getitem__(self, i):
        ...
        if self.token_type_ids is not None: d["token_type_ids"] = self.token_type_ids[i]
        return d, self.labels[i]

log("\nDataset tokenize ediliyor (bu birkaç dakika surebilir)...")
train_dl = DataLoader(ReviewDataset(train_df), batch_size=BATCH_SIZE, shuffle=True,  num_workers=0, pin_memory=False)
val_dl   = DataLoader(ReviewDataset(val_df),   batch_size=16,         shuffle=False, num_workers=0, pin_memory=False)
test_dl  = DataLoader(ReviewDataset(test_df),  batch_size=16,         shuffle=False, num_workers=0, pin_memory=False)
log(f"Train batches: {len(train_dl)} | Val batches: {len(val_dl)}")

# ── Model — mevcut checkpoint'ten yukle ──────────────────────────
if BASE_CKPT.exists():
    log(f"\nModel checkpoint'ten yukleniyor: {BASE_CKPT}")
    model = BertForSequenceClassification.from_pretrained(
        str(BASE_CKPT), num_labels=NUM_LABELS, ignore_mismatched_sizes=True
    )
else:
    log(f"\nCheckpoint bulunamadi, HuggingFace'den yukleniyor: {MODEL_NAME}")
...
log(f"Sinif agirliklari: {weights.tolist()}")
...
log(f"Adimlar: {total_steps} | Warmup: {warmup_steps} | Patience: {PATIENCE}")
...
results = {
    "berturk_v3": {
        ...
        "config": {
            ...
            "label_smooth": LABEL_SMOOTH, "data_version": "v4",
        },
    }
}
with open(RESULTS_DIR / "berturk_v3_results.json", "w", encoding="utf-8") as f:
    json.dump(results, f, indent=2, ensure_ascii=False)
log("Sonuclar: results/berturk_v3_results.json")
log("\nEGITIM TAMAMLANDI (BERTurk v3b).")
